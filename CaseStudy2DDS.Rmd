...
For a video presentation of this analysis, please see: [Video Presentation](https://youtu.be/1Z7Z2Z7Z2Z4)
...
A Naive Bayes model is a probabilistic model that is commonly used for classification tasks. It's based on Bayes' theorem, which states that the probability of a hypothesis (in this case, a classification) given some evidence (in this case, the features or variables) is proportional to the probability of the evidence given the hypothesis, multiplied by the prior probability of the hypothesis. Theorem: $$ P(A|B) = (P(B|A)P(A))/P(B) $$ The Naive Bayes model makes the assumption that the features are independent of each other, which is why it's called "Naive". This assumption is often violated in real-world data, but it's a useful model for classification tasks.
